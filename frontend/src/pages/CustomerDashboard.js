...
const CustomerDashboard = () => {
  ...
  const getStatusConfig = (status) => {
    const configs = {
      ...
      'picked_up': { color: 'bg-orange-500', icon: Truck, label: 'Picked Up', step: 5 },
      'on_the_way': { color: 'bg-orange-600', icon: Truck, label: 'On the Way', step: 6 },
  ...
  const OrderCard = ({ order, isActive }) => {
    ...
    return (
      <Card className={`mb-4 ${isActive ? 'border-2 border-orange-300 shadow-lg' : ''}`} data-testid={`order-card-${order.id}`}>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="text-right">
              ...
              <p className="font-bold text-lg text-orange-600">Total: ₹{order.total?.toFixed(2)}</p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50" data-testid="customer-dashboard">
      {/* Header */}
      <header className="bg-gradient-to-r from-orange-600 via-red-600 to-orange-700 text-white shadow-lg">
